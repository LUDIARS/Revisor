...
**ワーカーが 1 プロセスであることは、審査が 1 件ずつという意味ではない。** drain は
拾った job を互いに待たせずに走らせる。外側の同時審査数は worker 側 scheduler が、各審査の
高価な段階の同時実行数は段階ワーカープールが決める (`review-concurrency.md`)。ここを逐次に
すると、遅いモデルレビュー 1 件が空いているテスト / Anatomia / セキュリティのワーカーを
遊ばせたまま後続を止める。
