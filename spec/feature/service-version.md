...
「いまこのリポジトリが名乗る版」だから。版ファイルが次の minor (`2.5.0`) を宣言している
リポジトリでタグ優先のままだと、まだ公開していない `2.4.0` と答えてしまう。
...
**公開済みの版には、そこからの進み具合を build metadata として添える** (`2.4.0+336`)。

進み具合を `.revisor-version` の patch として持たせることはできない。あのファイルは追跡
されてはいるが `skip-worktree` で中身の変更は commit されず、origin には `uninitialized` が
載ったままになる。patch を積むと別マシンの checkout で消え、同じ commit を指していても
版が食い違う。だから状態には持たず、公開済みタグからの距離として毎回導出する — その数は
release state が既に数えている。

`2.4.336` にしないのは、それが「2.4.0 の 336 個目の patch リリース」を意味するから。patch
Release は 1 つも出していない。build metadata なら比較では無視され、「2.4.0 系である」ことが
保たれる。距離が 0 のとき、距離を取れないとき、代表値が走行版や `package.json` に落ちた
ときは何も添えない — 「何からの距離か」が言えないため。
