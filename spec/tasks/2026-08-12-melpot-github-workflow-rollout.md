...
しか通っていない。 MELPOT/PrivateGame を `github` workflow へ移行し、
...
- `revisor repo set-workflow MELPOT/PrivateGame github` を実行し、 `repo list` の
  workflow 列が `github` になる。
- KS で `revisor pr merge <n>` → plain push が実際の GitHub へ通り、 `publication` が
  `published` になる (保留に落ちない)。 落ちた場合は
  `revisor publish-pending --repository MELPOT/PrivateGame` で解消できることまで確認。
...
- 実行は MELPOT/PrivateGame の登録 checkout に対して行う (Revisor 側のコード変更は伴わない)
